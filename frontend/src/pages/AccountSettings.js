    ...
    useEffect(()=>{
        async function getUser(){
            ...
            if(user.data.role==='Student'){
                const deets= await axios.get(`/api/getStudentDetails?id=${user.data.id}`)
                setStudAccount(deets.data.details)
            }else{
                const deets= await axios.get(`/api/getTeacherDetails?id=${user.data.id}`)
                setStudAccount(deets.data.details)
            }
    ...
    const editDetails= async ()=>{
            ...
            if(indexSelected===0){
                ...
                if(!phoneNumError){
                    ...
                    const data= await axios.post('/api/editPhoneNum',{id:user.id, phoneNum:phoneNumHolder})
                    ...
                    if(!gNameError){
                        ...
                        const data= await axios.post('/api/editGuardianName',{id:user.id,guardianName:guardianNameHolder})
                ...
                if(!gContactError){
                    ...
                    const data= await axios.post('/api/editGuardianContact',{id:user.id,guardianContact:guardianContactHolder})
                    closeDialog()
                }
            }
