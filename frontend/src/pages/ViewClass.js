...
const ViewClass=()=>{
    ...
    useEffect(()=>{
      ...
      async function getUserApp(){
        ...
        if(user.data.role==='Teacher'){
            const classes = await axios.get(`/api/getTeacherClasses?teacherId=${user.data.id}`)
            dispatch({type:'SET_CLASSES',payload:classes})


            setClassState(classes.data)
        }else if(user.data.role==='Student'){
            const classes = await axios.get(`/api/getStudentClasses?studentId=${user.data.id}`)
            dispatch({type:'SET_CLASSES',payload:classes})
            setClassState(classes.data)
        }
      };
    ...
    const closeDeleteDialog=()=>{
     ...
     const deleteClass=()=>{
        ...
        try{
            axios.delete(`/api/deleteClass?id=${(state.classes.data)[index]._id}`).then(({data})=>{
            })
            closeDeleteDialog()
            setRefresh(true)
            setSuccess('success')
    ...
    const submitJoin= async(e)=>{
        ...
        if(enterClassCode){
            const joinData=await axios.post(`/api/joinClass`,{code:enterClassCode, studentId:user.data.id})
    ...
    const submitJoinTeacher= async(e)=>{
        ...
        if(enterClassCodeT){
            ...
            const joinData=await axios.post(`/api/joinClassTeacher`,{code:enterClassCodeT, teacherId:user.data.id})
            if(joinData.data.error){
    ...
    const leaveClass=()=>{
        ...
        axios.post(`/api/leaveClass`,{code:(state.classes.data)[index].classCode, studentId:user.data.id}).then(res =>{
